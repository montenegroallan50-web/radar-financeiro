...
export default function AlertasPage() {
  ...
  function AlertGroup({ label, items }: { label: string; items: typeof alerts }) {
    ...
    return (
      <div className="mb-3">
        <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-2 px-0.5">{label}</p>
        <div className="space-y-2">
          {items.map(a => {
            const s = typeStyle[a.type] || typeStyle['info'];
            return (
              <div key={a.id} className="rounded-2xl border shadow-sm overflow-hidden relative"
                ...
                <div className="pl-4 pr-3 pt-3 pb-2">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[12px] font-bold text-gray-800 leading-tight">{a.title}</p>
                      <p className="text-[11px] text-gray-500 mt-1 leading-relaxed">{a.desc}</p>
                      <p className="text-[10px] text-gray-400 mt-1">{a.time}</p>
                    </div>
                    <button onClick={() => dismiss(a.id)}
                      className="w-5 h-5 rounded-full bg-black/10 flex items-center justify-center text-[10px] text-gray-500 flex-shrink-0">
                      ✕
                    </button>
                  </div>
                  <div className="flex gap-2 mt-2.5">
                    ...
                    <button onClick={() => dismiss(a.id)}
                      className="px-3 py-1.5 rounded-lg text-[11px] font-semibold text-gray-500 border border-gray-200 bg-white">
                      Dispensar
                    </button>
                  </div>
                </div>
      ...
      <div className="relative flex items-center justify-between px-4 py-3 sticky top-0 z-10 overflow-hidden"
        ...
        {alerts.length > 0 && (
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full relative z-10"
            ...
            <span className="text-[11px] font-bold text-white">{alerts.length} ativo{alerts.length !== 1 ? 's' : ''}</span>
          </div>
        )}
      </div>
      ...
      <div className="flex bg-white border-b border-gray-200 shadow-sm">
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            className="flex-1 py-2.5 text-[10px] font-semibold whitespace-nowrap transition-all border-b-2 flex items-center justify-center gap-1"
...
{alerts.length === 0 ? (
              ...
              <>
                ...
                <button onClick={dismissAll}
                  className="w-full text-center text-[11px] font-semibold text-gray-400 py-3">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
              <div className="flex justify-between mb-4">
                <p className="text-[12px] font-semibold text-gray-700">Alertar ao atingir</p>
              </div>
              <div className="relative pt-9 pb-1">
                ...
                >
                  <div className="bg-[#0F6E56] text-white text-[12px] font-bold px-3 py-1 rounded-xl shadow-lg">
                    {thresh}%
                  </div>
              ...
              <div className="flex justify-between mt-3">
                {['50%','60%','70%','80%','90%','95%'].map(v => (
                  <span key={v} className="text-[9px] text-gray-400 font-medium">{v}</span>
                ))}
              </div>
            </div>
            ...
            {[
              ...
              <div key={section.title}>
                <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-2">{section.title}</p>
                <div className="space-y-2">
        ...
        {activeTab === 'hist' && (
          <div>
            <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-3">Últimos 30 dias</p>
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              {historico.map((h, i) => (
                <div key={i} className={`flex items-start gap-3 p-3 ${i < historico.length-1 ? 'border-b border-gray-50' : ''}`}>
                  ...
                  <span className="text-[10px] text-gray-400 whitespace-nowrap flex-shrink-0">{h.time}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 backdrop-blur-sm"
          ...
          <div className="bg-white w-full max-w-[430px] rounded-t-3xl p-6 shadow-2xl">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-[14px] font-bold text-gray-800 flex-1 leading-tight">{modalAlert.title}</h3>
            </div>
            <div className="bg-gray-50 rounded-2xl p-4 mb-6 border border-gray-100">
              <p className="text-[12px] text-gray-600 leading-relaxed">{modalAlert.detail}</p>
            </div>
